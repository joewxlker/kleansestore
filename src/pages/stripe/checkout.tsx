...
const Checkout: NextPage = (): JSX.Element => {
  ...
  const handleResponse = async () => {
    const itemData = items.map((x: ProductData) => {
      return { quantity: x.quantity, price: `${x.default_price.replace(/[^a-zA-Z0-9 _ ]/g, "")}` };
    });
      ...
      <Layout>
        <div className="w-screen h-screen flex flex-col justify-center items-center">
          <div className="w-full h-full flex-row flex justify-center items-center">
            ...
            <button onClick={e => handleResponse()} className="px-4 h-80 w-60 py-3 bg-grey text-white">
              Proceed to checkout
            </button>
          </div>
        </div>
