...
const HelpLayout: FC<HelpLayoutProps> = ({ children }): JSX.Element => {
    ...
    return (
        <>
            ...
            <Header />
            {history !== undefined && <div className='h-20 w-full flex flex-row'>
                {history.map((data) => {
                    return (
                        <>
                            {data !== '' && <span className='h-full w-32 p-5 flex flex-row justify-evenly items-center'>
                                <span className='w-1/2 flex flex-row justify-center'>
                                    <Link href={`${history.slice(0, history.indexOf(data) + 1).map(x => { if (x !== '') return (`/${x}`) }).join('')}`}>
                                        <a className='w-full' style={{ color: `${history[history.length - 1] === data ? 'rgba(0,0,0,0.5' : 'black'}` }}>
                                            <h4>{data}</h4>
                                        </a>
                                    </Link>
                                </span>
                                <span className='w-1/2 flex flex-row justify-center'>
                                    <h4 className='w-full text-center'>{history[history.length - 1] !== data &&
                                        <Image src='/images/ui-elements/angle-right-thin.svg' width={20} height={20} />
                                    }</h4>
                                </span>
                            </span>}
                        </>
    ...
    return (
        <div className='w-full bg-white shadow flex flex-row items-center'>
            <div className=' h-full w-1/2 p-5 flex flex-row justify-center items-center'>
                <Link href='/'><span className='p-2 cursor-pointer'>
                    <Image src='/images/ui-elements/arrow-left-long-thin.svg' width={20} height={20} />
                    <a className='px-5'>BACK TO KLEANSE</a></span></Link>
            </div>
            <div className=' h-full w-1/2 p-5 flex flex-col justify-center items-center'>
                <Image src='/images/kleanse-logos/kleanse-wing-in-text.svg' width={100} height={40} />
...
export const Footer: FC<FooterProps> = (): JSX.Element => {
    ...
    return (
        <div className='flex bg-grey p-5 text-white flex-row justify-center items-center' style={{ color: 'rgb(150,150,150)' }}>
            <p>Terms of use</p><p className='px-5'>|</p><p>Privacy</p>
        </div>
    );
}
