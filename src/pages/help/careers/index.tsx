...
const Careers: NextPage<CareerProps> = (props): JSX.Element => {
    ...
    const handleClick = () => {
        if (!data) { setData(!data); return setJobdata([]); };
        setData(!data);
        return setJobdata([{
        ...
        <>
            ...
            <HelpLayout>
                <div className='lg:flex-row md:flex-col flex-col w-screen lg:h-screen md:h-screen flex items-start'>
                                ...
                                {jobData.map(({ title, description, timePosted, location }) => {
                                    return (
                                        <div key={title} className='flex flex-col m-3 w-full justify-evenly'>
                                            ...
                                            <button className='bg-grey text-white py-4' onClick={e => { setApplicationForm(true); setParams({ title: title }) }}>APPLY</button>
                                        </div>)
                                })
                                }
                            </> :
                    ...
                    <button className='bg-grey text-white' onClick={e => handleClick()}>Toggle Job data</button>
                    <div className='h-full lg:h-4/6 lg:w-4/6 md:w-full w-full flex flex-col justify-center items-center py-12 lg:justify-start'>
                        <span className='w-4/6 h-4/6 flex flex-col justify-evenly items-evenly'>
                            ...
                            <p>Influential, innovative and progressive, Kleanse is reinventing a wholly modern approach
                                ...
                                world&#39;s most desirable fashion houses. Eclectic, contemporary, romantic—Kleanse products</p>
                        </span>
                    </div>
                    {applicationForm &&
                        <Layover>
                            <div className='flex flex-col items-center'>
                                <header className='h-12 w-full flex flex-row justify-end items-center px-8'>
                                    <button onClick={e => setApplicationForm(false)}><p>Changed your mind about applying?</p></button>
                                </header>
