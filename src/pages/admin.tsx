      ...
      <div className="container mb-6 flex flex-row justify-evenly items-center flex-wrap rounded-3xl">
          ...
          listData={[
            {
              title: "Read",
              data: [
                "$0.10 / million for the first 50 million bytes per day",
                "Next 500 million bytes: $0.05 / million bytes",
                "Reads thereafter: $0.01 / million bytes",
              ],
            },
        ...
        >
          <MongoModule
            ...
            setActive={(index: number) => setMongoIndex(index)}
          />
        </Wrapper>
      </div>
    </>
  ...
  return (
    <div ref={DIVELEMENT} className="w-full h-full flex justify-center items-center flex-col text-white ">
      ...
      <div className="h-[20%] w-full flex flex-row justify-evenly items-center">
        <button
          className={`${type === "reads" && "bg-accent"} w-[33.333333%] h-full translate-y-1`}
          onClick={e => {
            setType("reads");
            handleSetActive("reads");
          }}
        >
          Reads
        </button>
        <button
          className={`${type === "writes" && "bg-accent"} w-[33.333333%] h-full translate-y-1`}
          onClick={e => {
            setType("writes");
            handleSetActive("writes");
          }}
        >
          Writes
        </button>
        <button
          className={`${type === "storage" && "bg-accent"} w-[33.333333%] h-full translate-y-1`}
          onClick={e => {
            setType("storage");
            handleSetActive("storage");
          }}
        >
          Storage
        </button>
