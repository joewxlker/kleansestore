  ...
  if (!items || items.length === 0)
    return (
      <div id="hardfadein" className="w-full flex flex-col justify-center items-center">
        ...
        <button className="bg-grey px-3 py-2 text-white" onClick={() => (window.location.href = "/products/all-products")}>
          Shop Now
        </button>
      </div>
    );
      ...
      <div className="w-full flex flex-col justify-center items-center z-10 bg-white">
                  ...
                  <span className="flex w-1/2 flex-row justify-center items-center">
                    <div className="flex w-4/6 flex-col jusitfy-center items-center">
                      <button
                        ...
                        onClick={() => {
                          handleQuantities(data, true);
                        }}
                        style={{ opacity: `${data?.quantity === 10 ? "50%" : "100%"}` }}
                      >
                        +
                      </button>
                      <h3>{data.quantity}</h3>
                      <button
                        ...
                        onClick={() => {
                          handleQuantities(data, false);
                        }}
                        style={{ opacity: `${data.quantity === 1 ? "50%" : "100%"}` }}
                      >
                        -
                      </button>
                    </div>
                    <button className="h-16 w-16 text-grey m-2 bg-salmon rounded" onClick={() => handleRemoveItem(data)}>
                      X
                    </button>
                  </span>
                </div>
        ...
        {items?.length !== 0 && (
          <Link href="/stripe/checkout">
            <span className="w-[90%]">
              <button className="mt-5 py-2 bg-salmon rounded margin-4 w-[100%]" onClick={() => ""}>
                Checkout
              </button>
            </span>
          </Link>
